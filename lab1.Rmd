...
movies2 <- movies %>%
  mutate(
    # Género principal = primer género antes del "|"
    main_genre = ifelse(is.na(genres) | genres == "", "Sin género", sub("\\|.*", "", genres)),
    
    # Parsear releaseDate (si viene en formato tipo "YYYY-MM-DD")
    release_dt = suppressWarnings(ymd(releaseDate)),
    
    # Si no se pudo parsear releaseDate, usar releaseYear como fallback (31 dic de ese año)
